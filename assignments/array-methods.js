...
// Problem 1
// The event organizer wants a list of everyone's names and email addresses to keep them updated on inclement weather, and future events.

let emails = runners.map((email) => `${email.first_name} ${email.last_name} - ${email.email}`);

console.log(emails);

// Problem 2
// The event organizer is going to make a banner for all the sponsors of the event, and needs the duplicates filtered out. This will be easiest to see sorted in alphabetical order.

let sponsors = [];
runners.forEach((runners) => {
	if (sponsors.indexOf(runners.company_name) === -1) {
		sponsors.push(runners.company_name);
	}
});
sponsors.sort();
...
// Problem 3
// The event organizer needs to group runners into timed waves to be able to fit everyone in. They want an alphabetical list of all the runners sorted by their last name, to be able to group them easily.

const lastNameFirst = [];
runners.forEach((name) => lastNameFirst.push(`${name.last_name}, ${name.first_name};`));
lastNameFirst.sort();
